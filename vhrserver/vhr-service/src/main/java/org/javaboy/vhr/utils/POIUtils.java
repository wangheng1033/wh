...
public class POIUtils {
    ...
    public static ResponseEntity<byte[]> resume2Excel(List<Resume> list) {
        ...
        HSSFRow r0 = sheet.createRow(0);
        String[] headers = {"编号", "姓名", "性别", "年龄", "学历", "工作年限", "电话", "邮箱", "期望薪资", "求职岗位", "简历状态"};
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            row.createCell(8).setCellValue(resume.getExpectedSalary() != null ? resume.getExpectedSalary().doubleValue() : 0);
            row.createCell(9).setCellValue(resume.getPosition() != null ? resume.getPosition().getName() : "");
            row.createCell(10).setCellValue(resume.getStatus());
        }
        
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        HttpHeaders httpHeaders = new HttpHeaders();
        try {
            httpHeaders.setContentDispositionFormData("attachment", new String("简历表.xls".getBytes("UTF-8"), "ISO-8859-1"));
    ...
    public static List<Resume> importResumes(MultipartFile file) {
                            ...
                            default:
                                switch (k) {
                                    ...
                                    case 5:
                                        ...
                                        break;
                                    case 8:
                                        resume.setExpectedSalary(BigDecimal.valueOf(cell.getNumericCellValue()));
                                        break;
                                }
                                break;
                        }
